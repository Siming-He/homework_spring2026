...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discounts = np.power(self.gamma, np.arange(rewards_arr.shape[0], dtype=np.float32))
        total_return = float(np.sum(discounts * rewards_arr))
        return np.full(rewards_arr.shape[0], total_return, dtype=np.float32)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        running_sum = 0.0
        for t in reversed(range(rewards_arr.shape[0])):
            running_sum = float(rewards_arr[t]) + float(self.gamma) * running_sum
            rtg[t] = running_sum
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            with torch.no_grad():
                ...
                values = ptu.to_numpy(self.critic(obs_tensor)).reshape(-1)
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    delta = rewards[i] + gamma * values[i + 1] * not_terminal - values[i]
                    advantages[i] = delta + gamma * lam * not_terminal * advantages[i + 1]
